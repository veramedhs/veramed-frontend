...
// --- Type Definitions ---
// No longer need to export ReviewFormData as it's handled internally
interface ReviewModalProps {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  onSuccess: () => void; // A function to call when submission is successful
}
...
export const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onOpenChange, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // --- FormData is CRITICAL for file uploads ---
    ...
    if (imageFile) {
      // This key "profileImage" MUST match what your backend Multer middleware expects
      formData.append('profileImage', imageFile);
    }
    
    // --- API Call with react-hot-toast ---
    ...
    try {
      ...
      resetForm();
      onSuccess(); // Notify parent component of success (e.g., to close the modal)
    } catch (error) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="review" value={review} onChange={(e) => setReview(e.target.value)} placeholder="Tell us about your experience..." rows={4} required disabled={isSubmitting} />
            </div>

            <div className="space-y-2">
              <Label>Upload Photo (Optional)</Label>
              <Input id="image-upload" type="file" accept="image/*" onChange={handleImageChange} className="hidden" disabled={isSubmitting} />
              <Label htmlFor="image-upload" className={`flex items-center gap-2 p-2 border-2 border-dashed rounded-md transition-colors ${isSubmitting ? 'cursor-not-allowed bg-muted/50' : 'cursor-pointer hover:border-primary hover:bg-primary/10'}`}>
