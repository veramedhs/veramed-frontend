...
const PostTreatment: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.files.length > 5) newErrors.files = "You can upload up to 5 files only.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSubmitted(true);
        toast.success("Request submitted successfully! Our care team will contact you shortly.");
      ...
      <div className="bg-gray-50 min-h-screen py-16 px-4 md:px-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start max-w-6xl mx-auto mb-16">
          ...
          <div className="rounded-xl overflow-hidden shadow-md">
            <img src={postTreatmentImg} alt="Post Treatment Follow Up" className="w-full h-full object-cover" />
          </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <label htmlFor="fileUpload" className="cursor-pointer">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-500 text-sm">
                          or drag and drop<br />Image, PDF, DOC/DOCX (max. 10MB)
                        </p>
                      </div>
                    </label>
                  </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-200 space-y-4">
            <img src={img1} className="w-full h-60 object-cover rounded-xl shadow-md" />
            <img src={img2} className="w-full h-60 object-cover rounded-xl shadow-md" />
            <img src={img3} className="w-full h-60 object-cover rounded-xl shadow- md" />
          </div>
        </div>
      </div>
